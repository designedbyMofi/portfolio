  ...
  return (
    <article className="case-study" aria-labelledby="vurt-title">
      ...
      <section className="case-study__intro case-study__section" id="vurt-intro">
        ...
        <div className="case-study__intro-copy"><h2>Intro — A redesign for a two-sided exchange marketplace.</h2><p>Vurt began as QPay, a mobile-first exchange app for people moving money across currencies. The first version proved the need, but it made a high-stakes transaction feel harder than it needed to be.</p><p>The redesign turned that early product into a P2P marketplace: customers could compare offers and choose who to trade with, while Vurt handled the coordination and risk underneath.</p></div>
      </section>

      <section className="case-study__simple-section" id="vurt-metrics"><div><span className="case-study__section-number">01</span><h2>Metrics — The clearest signal came from onboarding.</h2></div><div><p>Onboarding completion rose from 26% to 67.6% after the redesign—a 160% increase.</p><p>That was the clearest evidence that reducing the first-use burden was doing its job. The Beta’s early traction gave us a second signal: people were willing to return when the exchange felt useful and dependable.</p></div></section>
      <section className="case-study__simple-section" id="vurt-role"><div><span className="case-study__section-number">02</span><h2>Role — I owned the product direction from strategy to handoff.</h2></div><div><p>I led the product strategy, UX, UI, and design system across the customer and exchanger experiences.</p><p>The team was one product manager, three developers, and three designers. I handled the core product direction and worked closely with stakeholders throughout. The other designers extended the system across mobile viewports and features such as KYC, bank-account listing, and exchanger ads.</p></div></section>

      <section className="case-study__simple-section" id="vurt-opportunity"><div><span className="case-study__section-number">03</span><h2>Opportunity — Make exchange easier to compare and safer to complete.</h2></div><div><p>New startups were offering similar solutions, while traditional exchangers were still relying on WhatsApp, Telegram, and in-person conversations. The opportunity was bigger than a visual refresh: Vurt could give both sides a better way to do business.</p><p>For customers, that meant clearer choices and a faster path to payment. For exchangers, it meant bringing existing customers online without asking them to abandon the relationships they had already built.</p></div></section>
      <section className="case-study__simple-section" id="vurt-problem"><div><span className="case-study__section-number">04</span><h2>Problem — The core problem was not finding a rate. It was trusting the exchange.</h2></div><div><p>A customer needed confidence at the exact moment money left their account: what would they receive, who would receive their deposit, how long would it take, and what protection existed if the transaction failed?</p><p>The exchanger had a related problem. Manual transfers created opening hours, delays, and exposure to risk. The product had to make the exchange feel immediate for the customer while keeping the underlying transaction controlled.</p></div></section>
      <section className="case-study__simple-section" id="vurt-context"><div><span className="case-study__section-number">05</span><h2>Context — We used a small beta to test the marketplace model.</h2></div><div><p>The Beta was the first time we put multiple exchangers in the same product. Three internal exchangers supplied rates at different times of day, but all appeared under the QPay name.</p><p>Over two weeks, 400+ people used the platform and 109 came back for another exchange. With almost no marketing, the experiment showed that customers were willing to compare offers—and that the marketplace model could support repeat behaviour.</p></div></section>
      <section className="case-study__simple-section" id="vurt-approach"><div><span className="case-study__section-number">06</span><h2>Process — I treated the redesign as a product-system problem.</h2></div><div><p>I started with the moments where confidence could break: account creation, choosing an offer, making payment, waiting for resolution, and managing an exchange as a provider.</p><p>We combined competitor analysis, Beta behaviour, stakeholder input, and our own experience as customers and exchangers. I then translated those findings into the design system, interaction patterns, and implementation decisions, working with the PM and developers throughout.</p></div></section>
      <img className="case-study__full-image" src="/assets/vurt-exchange-mobile.png" alt="Vurt exchanger cards on mobile" data-reveal />
      <section className="case-study__simple-section" id="vurt-solution"><div><span className="case-study__section-number">07</span><h2>Solution — Put the decision in one clear place.</h2></div><div><p>The exchanger card became the centre of the marketplace. It answers the questions a customer has before committing: what rate am I getting, who am I dealing with, and how will the exchange work?</p><p>Rate, limits, response time, trade count, reviews, and verification status sit in one hierarchy. Buy and Generate payment link stay close to the offer, so the action follows the decision instead of starting another search.</p><p>Payoneer, PayPal, and Access Bank are separated into tabs. Desktop supports quick comparison across offers; mobile keeps the same information legible in a single scroll.</p></div></section>
      <section className="case-study__simple-section" id="vurt-onboarding"><div><span className="case-study__section-number">08</span><h2>Onboarding — Let people understand the exchange before asking for commitment.</h2></div><div><p>The original flow asked for an account before users had seen enough value. In the new flow, a customer can choose an exchanger, enter an amount, and see the expected return before signing up.</p><p>Registration appears when payment details are needed. Exchange links take the same idea further: an exchanger can send an existing customer directly to their offer, turning a WhatsApp or Telegram conversation into a structured transaction.</p></div></section>
      <section className="case-study__simple-section" id="vurt-trust"><div><span className="case-study__section-number">09</span><h2>Trust — Design the system state, not just the happy path.</h2></div><div><p>The key question was simple: what stops an exchanger from taking the money and disappearing?</p><p>Vurt uses an automated escrow system. Customer funds move into an account provided through a partner bank, and the equivalent amount is held until the exchange is confirmed. If resolution fails, the exchanger cannot withdraw the funds.</p><p>The interface makes that protection legible through verified profiles, trade history, response times, reviews, and a progress card that shows where the exchange is. The system does the heavy lifting; the UI gives the customer enough evidence to keep moving.</p></div></section>
      <img className="case-study__full-image" src="/assets/vurt-overview.png" alt="Vurt dashboard showing exchange, transactions, and account actions" data-reveal />
      <section className="case-study__simple-section" id="vurt-platform"><div><span className="case-study__section-number">10</span><h2>Platform — Move to web before adding more surface area.</h2></div><div><p>We chose web because the first interaction needed to be easy to share and easy to access. Asking someone to download an app before their first exchange was unnecessary friction.</p><p>The dashboard gives customers a home for their money exchanged, transactions, settings, and recent activity. It also gives exchangers room to manage KYC, bank accounts, offers, and generated links.</p></div></section>
      <section className="case-study__simple-section" id="vurt-invoice-links"><div><span className="case-study__section-number">11</span><h2>Invoice links — Give freelancers a simple way to get paid.</h2></div><div><p>Several test customers asked for a way to collect payments from their own clients. They did not want to explain exchange details every time or ask someone to download an app for a single payment.</p><p>Invoice links let a freelancer create a payment request with the information a proper invoice needs: the amount, currency, exchange rate, payment details, and due date. Vurt generates a shareable link that the client can open, review, and pay from the web.</p><p>The payment first goes directly into the exchanger’s account. Vurt then reconciles the payment through the exchange flow, converts the value, and settles the exchanged amount into the freelancer’s receiving account. This keeps the client’s payment simple while the platform handles the currency conversion and settlement underneath.</p></div></section>
      <section className="case-study__simple-section" id="vurt-impact"><div><span className="case-study__section-number">12</span><h2>Impact — The redesign improved the first step.</h2></div><div><p>Onboarding completion rose from 26% to 67.6%, a 160% increase, before the current relaunch.</p><p>The Beta also gave us meaningful early traction: 400+ users and 109 repeat users in two weeks with almost no marketing. The current product is preparing for launch with a stronger foundation for comparison, trust, and reliable resolution.</p></div></section>
      <section className="case-study__simple-section" id="vurt-feedback"><div><span className="case-study__section-number">13</span><h2>Feedback from users — The strongest feedback was behavioural.</h2></div><div><p>The most useful feedback was not a comment. It was what people did next. Users returned because they could find a rate that worked for them without giving up the familiarity of their exchanger.</p><p>That behaviour reinforced the product direction: customers wanted control over the price, but they also needed the experience to feel safe, visible, and predictable from payment to resolution. The invoice-link request showed the same pattern: customers were telling us where the platform could remove work beyond the core exchange.</p></div></section>
      <section className="case-study__simple-section" id="vurt-lessons"><div><span className="case-study__section-number">14</span><h2>Lessons learnt — Design the system behind the screen.</h2></div><div><ul className="case-study__lessons-list"><li>Trust is a product behaviour, not a visual layer.</li><li>In a two-sided marketplace, every customer decision affects another user.</li><li>Progressive disclosure can remove friction without hiding important information.</li><li>Operational constraints should shape the experience from the start.</li><li>Repeat behaviour is stronger evidence than positive feedback alone.</li><li>A web-first product can validate demand before adding the cost of a mobile app.</li><li>A design system gives a growing team a shared way to make decisions.</li><li>The best adjacent features often come directly from what customers are already trying to do.</li></ul></div></section>
      <div className="case-study__before-after"><figure><img src="/assets/vurt-qpay-signup.png" alt="QPay signup screen from the first version" data-reveal /><figcaption>QPay v1 · 2021–22</figcaption></figure><figure><img src="/assets/vurt-signup.png" alt="Vurt simplified signup screen" data-reveal /><figcaption>Vurt · 2026</figcaption></figure></div>
      </div>
      </div>
      <div className="case-study__closing-line" id="vurt-credits"><span>Credits — Designed by Mofifoluwa Olawuyi</span><a href="/projects" onClick={(event) => { event.preventDefault(); window.history.pushState({}, '', '/projects'); window.dispatchEvent(new PopStateEvent('popstate')); }}>Back to projects</a></div>
    </article>
  );
}
