...
function VurtCaseStudy() {
  ...
  return (
    <article className="case-study" aria-labelledby="vurt-title">
      ...
      <nav className="case-study__toc" aria-label="Case study sections">{navSections.map((section) => <a key={section.id} href={`#${section.id}`} className={section.number === '08.1' ? 'is-active' : ''}><b>{section.number.split('.')[0]}</b><span>{section.nav}</span></a>)}</nav>
